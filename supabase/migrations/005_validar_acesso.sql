-- RPC ao abrir o app: garante linha por email e devolve estado (sem gastar uso).

drop function if exists public.validar_acesso(text, text);
...
begin
  ...
  if vtipo = 'pago' then
    -- vazio na base: primeira abertura com device — vincular
    ...
    -- já há device na base: tem de coincidir
...
comment on function public.validar_acesso(text, text) is
  'Abertura app: garante UPSERT por email; pago valida/dispositivos; teste apenas devolve quotas.';
